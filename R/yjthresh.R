# ================================= yjthresh ==================================
#
#' Threshold selection in the i.i.d. case, assisted by Yeo-Johnson
#' transformation
#'
#' Uses the function \code{\link{ithresh}} to assist in the selection of
#' an extreme value threshold after transforming the data with a Yeo-Johnson
#' transformation.  The general aim is to seek a Yeo-Johnson transformation
...
#' @param lambda A numeric vector containing values of the Yeo-Johnson
...
#' @return An object (list) of class \code{"yjthresh"}, containing the
#'   components
#'   \itemize{
...
#'     \item{\code{lambda}:} The argument \code{lambda} to \code{yjthresh}.
...
#'     \item{\code{data}:} The argument \code{data} to \code{yjthresh}
...
#'     \item{\code{call:}} The call to \code{yjthresh}.
#'   }
...
#' gom_lambda <- do.call(yjthresh, c(gom_args, prior_args))
#'
...
#' ns_lambda <- do.call(yjthresh, c(ns_args, prior_args))
#'
...
#' log_exp <- do.call(yjthresh, c(exp_args, prior_args))
...
yjthresh <- function(data, probs, lambda, ..., n_v = 1, npy = NULL,
  ...
  for (i in 1:n_lambda) {
    ...
    yj_data <- yj(raw_data, lambda = lambda[i])
    yj_u_vec <- yj(u_vec, lambda = lambda[i])
    yj_v_vec <- yj(v_vec, lambda = lambda[i])
    temp <- yjcv_fn(data = yj_data, u_vec = yj_u_vec, v_vec = yj_v_vec,
...
yjcv_fn <- function(data, u_vec, v_vec, n_u, n_v, use_rcpp, raw_data, lambda,
                    ...) {
  ...
  for (i in 1:n_u){
    ...
    # then try trans = "BC".  This tends to be slower but the Yeo-Johnson
    ...
    pred_perf[i, which_v] <- yjbloocv(z = data, theta = theta,
...
yjbloocv <- function(z, theta, theta_rm, u1, u2_vec, z_max, z_rm, n,
